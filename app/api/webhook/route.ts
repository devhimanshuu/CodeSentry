...
export async function POST(req: NextRequest) {
		...
		if (action === "created" || action === "added") {
			const dbInstallation = await prisma.installation.upsert({
				...
				create: {
					...
					accountAvatar: installation.account.avatar_url,
					repositorySelection: installation.repository_selection,
					permissions: installation.permissions,
			...
			if (payload.repositories) {
				for (const repo of payload.repositories) {
					await prisma.repository.upsert({
						...
						update: { installationId: dbInstallation.id },
						create: {
							githubId: repo.id,
							name: repo.name,
	...
	try {
		...
		// Fetch changed files
		const files = await fetchPRFiles(repo.full_name, pr.number, installationId);
		...
		await postPRComment(repo.full_name, pr.number, comment, installationId);
