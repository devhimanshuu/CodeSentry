...
export async function runAIReview(params: {
	...
	const issuesSummary = ruleIssues
		.map(
			(i) =>
				`[${i.severity.toUpperCase()}][${i.category}] ${i.title}: ${i.description}`,
		)
		.join("\n");
...
function generateFallbackReview(
	...
	if (persona === "roast" && issues.length > 3) {
		summary +=
			" Whoever wrote this needs to re-read the OWASP top 10... and possibly a career counselor.";
	}
	...
	return {
		...
		confidence: 0.6,
		additionalInsights: [
			"AI review skipped — no API key configured. Add GROQ_API_KEY to .env.",
		],
	};
}
