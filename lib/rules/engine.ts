...
export type Category = "security" | "performance" | "logic" | "style" | "maintainability";
...
const SECURITY_PATTERNS = [
  ...
  },
  {
    ...
    suggestion: "Store API keys in environment variables (e.g., process.env.API_KEY).",
    severity: "critical" as Severity,
  },
  {
    ruleId: "SEC-003",
  ...
  },
  {
    ...
    suggestion: "Sanitize user input with DOMPurify before using dangerouslySetInnerHTML.",
  ...
  },
  {
    ...
    suggestion: "Remove logs containing sensitive information before deploying.",
    severity: "high" as Severity,
  },
  {
    ruleId: "SEC-007",
    ...
    suggestion: "Use crypto.getRandomValues() or Node's crypto.randomBytes() for security tokens.",
    severity: "medium" as Severity,
  },
];
...
const PERFORMANCE_PATTERNS = [
  {
    ...
    pattern: /SELECT\s+\*/gi,
    title: "SELECT * Usage",
    description: "Using SELECT * fetches all columns, which can be inefficient.",
  ...
  },
  {
    ...
    description: "Using await inside forEach doesn't work as expected and can cause performance issues.",
...
const MAINTAINABILITY_PATTERNS = [
  ...
  },
  {
    ...
    suggestion: "Fix the underlying type error or use a more specific suppression with a comment.",
    severity: "medium" as Severity,
  },
];
...
function checkFileLevel(file: DiffFile): RuleIssue[] {
  ...
  // Binary / lock file
  if (file.filename.endsWith("package-lock.json") || file.filename.endsWith("yarn.lock")) {
    issues.push({
      ruleId: "MAINT-004",
      severity: "info",
      category: "maintainability",
      title: "Lock File Modified",
      description: "Package lock file was modified — verify dependency changes are intentional.",
...
export function runRuleEngine(files: DiffFile[]): RuleIssue[] {
  ...
  for (const file of files) {
    ...
    const allPatterns = [
      ...SECURITY_PATTERNS.map((p) => ({ ...p, category: "security" as Category })),
      ...PERFORMANCE_PATTERNS.map((p) => ({ ...p, category: "performance" as Category })),
      ...MAINTAINABILITY_PATTERNS.map((p) => ({ ...p, category: "maintainability" as Category })),
